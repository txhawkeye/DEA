    ...
    public class RequireRank3Attribute : PreconditionAttribute
    {
        public override async Task<PreconditionResult> CheckPermissions(ICommandContext context, CommandInfo command, IDependencyMap map)
        {
            ...
            using (var db = new DbContext())
            {
                ...
                if (context.Guild.GetRole(guild.Rank3Id) == null)
                ...
                if (user.RoleIds.All(x => x != guild.Rank3Id))
                    return PreconditionResult.FromError($"You do not have the permission to use this command.\nRequired role: {context.Guild.GetRole(guild.Rank3Id).Mention}");
                if (await userRepo.GetCashAsync(user.Id) < Config.RANK3)
