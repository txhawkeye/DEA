...
namespace DEA.Modules
{
    public class Gambling : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task Gamble(float bet, int odds, float payoutMultiplier)
        {
            var userRepo = new UserRepository(_db);
            var Cash = await userRepo.GetCash(Context.User.Id);
            if (bet > Cash) throw new Exception("You do not have enough money");
            else if (bet < 5) throw new Exception("Lowest bet is $5");
            else if (bet < Cash / 10) throw new Exception($"The lowest bet is 10% of your total cash, that is {(Cash / 10).ToString("N2")}.");
            else {
